...
async function SelectBoXGet() {
  await axios.get('http://192.168.2.7/hostlist/', {
            headers: {
...
function url_group21(selected_Host){
  ...
  async function signin(){ // await 사용하기위해 async 사용   로그인
    ...
    await axios.post('http://192.168.2.7/accounts/login/', list_data) // post 조건이 완전히 완료될때까지 기다리라는 await
            .then(response => {
    ...
    async function log_out(){  //로그아웃
        ...
        await axios.post('http://192.168.2.7/accounts/logout/', data)
        .then(response => {
